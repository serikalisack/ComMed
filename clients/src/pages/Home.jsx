...
function Home() {
  ...
  return (
    <div className="bg-[#1a1d24] scrollbar-hide z-10 h-[100vh] lg:w-[90%] lg:mx-auto overflow-y-hidden shadow-2xl">
      <div className='flex'>
        {!showProfile ? (
          <div className="md:flex md:flex-col min-w-[360px] h-[100vh] md:h-[98.6vh] bg-[#1a1d24] relative">
            <div className='h-[61px] px-4'>
              <div className='flex'>
                <a className='flex items-center relative -top-4 block h-[90px]' href='/'>
                  <h3 className='text-[20px] text-[#FFD700] font-body font-extrabold tracking-wider'>
                    Messages
                  </h3>
                </a>
              </div>
              <div className='absolute top-4 right-5 flex items-center gap-x-3'>
                <button onClick={() => dispatch(setShowNotifications(!showNotifications))}>
                  ...
                  />
                  {showNotifications ? (
                    <RiNotificationBadgeFill style={{ width: "25px", height: "25px", color: "#FFD700" }} />
                  ) : (
                    <BiNotification style={{ color: "#FFD700", width: "25px", height: "25px" }} />
                  )}
                </button>
                <div
                  className={`${
                    showNotifications
                      ? "overflow-y-scroll scrollbar-hide tracking-wide absolute top-10 -left-32 z-10 w-[240px] bg-[#1a1d24] px-4 py-2 shadow-2xl"
                      : "hidden"
                  }`}
                >
                  ...
                  {notifications.map((e, index) => (
                    <div
                      ...
                      key={index}
                      className='text-[12.5px] text-[#f0f0f0] px-2 cursor-pointer'
                    >
                      {e.chatId.isGroup
                ...
                >
                  <img
                    ...
                    alt=""
                  />
                  <IoIosArrowDown style={{ color: "#FFD700", height: "14px", width: "14px" }} />
                </button>
              </div>
            </div>
            <div className='-mt-6 relative pt-6 px-4'>
              <form onSubmit={(e) => e.preventDefault()}>
                <input
                  ...
                  className='w-[99.5%] bg-[#3c475d] text-[#f0f0f0] tracking-wider pl-9 py-[8px] rounded-[9px] outline-0'
                  type="text"
                  name="search"
                  placeholder="Search"
                />
              ...
              <div className='absolute top-[36px] left-[27px]'>
                <BsSearch style={{ color: "#FFD700" }} />
              </div>
              <Group />
              <div
                style={{ display: search ? "" : "none" }}
                className='h-[100vh] absolute z-10 w-[100%] left-[0px] top-[70px] bg-[#1a1d24] flex flex-col gap-y-3 pt-3 px-4'
              >
          ...
          <Profile className="min-w-[100%] sm:min-w-[360px] h-[100vh] bg-[#1a1d24] shadow-xl relative" />
        )}
        <Chat className="chat-page relative lg:w-[100%] h-[100vh] bg-[#1a1d24]" />
      </div>
    </div>
  );
}
